...
/** @todo Refactor: simplify default dragStartPredicate. */
/** @todo Refactor: Get rid of Hammer.js or try to make it faster at least. */
/** @todo Bug: a parent item can't be dragged if a child is being dragged. */
/** @todo UX: When an item is being released but it's container position is
 changed before finishing, there's a nasty position jump.  */
...
ItemDrag.prototype.stop = function() {
  ...
  // immediately.
  if (this._isMigrating) return this._finishMigration(this._lastEvent);
...
ItemDrag.prototype._finishMigration = function() {
  ...
  item._animate = new ItemAnimate(item, element);
  item._animateChild = new ItemAnimate(item, item._child);
  ...
  setStyles(item._child, targetSettings.visibleStyles);
